...
def main():
    ...
    try:
        ...
        # 4. Start MTProxy with correct parameters (use -M 0 to avoid PID assertion error)
        print("\n=== Starting MTProxy ===")
        # Use -M 0 (single process) to avoid PID assertion error with large PIDs
        cmd = f"cd /root/MTProxy && nohup ./objs/bin/mtproto-proxy -u nobody -p 19196 -H 19196 -S {secret} --aes-pwd /root/proxy-secret /root/proxy-multi.conf -M 0 > /root/mtproxy.log 2>&1 &"
        run_command(ssh, cmd)
