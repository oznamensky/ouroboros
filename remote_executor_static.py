...
class RemoteExecutorHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == '/health':
            ...
            self.send_header('Content-Type', 'application/json')
            self.end_headers()
            self.wfile.write(json.dumps({'status': 'ok', 'service': 'remote-executor-static'}).encode('utf-8'))
        else:
            self.send_error(404, "Not Found")
    
    def log_message(self, format, *args):
        """Custom log format"""
        print(f"[RemoteExecutorStatic] {self.address_string()} - {format % args}")

def run_server():
    """Start the HTTP server"""
    with socketserver.TCPServer((HOST, PORT), RemoteExecutorHandler) as httpd:
        print(f"Remote Executor Static Server started on http://{HOST}:{PORT}")
        print(f"Authentication token: {STATIC_TOKEN}")
        print(f"Use: POST http://localhost:{PORT}/?token={STATIC_TOKEN}")
...
if __name__ == '__main__':
    print("Remote SSH Executor (Static) - Starting...")
